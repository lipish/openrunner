...
use super::{RunStore, Run, RunStatus, RunEvent, MessageDelta, RunCompleted, RunFailed, CompletedMessage};
...
impl RunManager {
    ...
    pub fn create_run(
        ...
        let run_id = format!("run_{}", Uuid::new_v4().to_string().replace("-", "")[..12].to_string());
        self.store.create(run_id.clone(), user_id.to_string(), session_id, input_text.to_string());
        run_id
    }
